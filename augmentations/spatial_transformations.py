...
import numpy as np
from augmentations.utils import create_zero_centered_coordinate_mesh, elastic_deform_coordinates, \
    interpolate_img, \
    rotate_coords_2d, rotate_coords_3d, scale_coords, resize_segmentation, resize_multichannel_image, \
    elastic_deform_coordinates_2
from augmentations.crop_and_pad_augmentations import random_crop as random_crop_aug
from augmentations.crop_and_pad_augmentations import center_crop as center_crop_aug
from scipy.ndimage import map_coordinates


def augment_rotation_alone_axis(img, a_x, a_y, a_z, mode='constant', cval=-2048):
    coords = create_zero_centered_coordinate_mesh(img.shape)
    coords = rotate_coords_3d(coords, a_x, a_y, a_z)
    order = 3
    print("cval = ", cval)
    for d in range(len(img.shape)):
        coords[d] += ((np.array(img.shape).astype(float) - 1) / 2.)[d]
        print("axis min: ", coords[d].min())
    new_img = map_coordinates(img.astype(float), coords, order=order, mode=mode, cval=cval).astype(img.dtype)
    return new_img

# Add by Ding
def translation_alone_RAS_LPI(d, direction, trans_step):
    assert (trans_step >= 0)
    assert len(d.shape) in [3, 4]
    assert direction in ["R", "A", "S", "L", "P", "I"]
    if trans_step == 0:
        return d
...
def augment_mirroring(sample_data, sample_seg=None, axes=(0, 1, 2), p=1):
    if (len(sample_data.shape) != 3) and (len(sample_data.shape) != 4):
        raise Exception(
            "Invalid dimension for sample_data and sample_seg. sample_data and sample_seg should be either "
            "[channels, x, y] or [channels, x, y, z]")
    if 0 in axes and np.random.uniform() < p:
        sample_data[:, :] = sample_data[:, ::-1]
        if sample_seg is not None:
            sample_seg[:, :] = sample_seg[:, ::-1]
    if 1 in axes and np.random.uniform() < p:
        sample_data[:, :, :] = sample_data[:, :, ::-1]
        if sample_seg is not None:
            sample_seg[:, :, :] = sample_seg[:, :, ::-1]
...
def augment_spatial(data, seg, patch_size, patch_center_dist_from_border=30,
                    ...
                    do_scale=True, scale=(0.75, 1.25), border_mode_data='nearest', border_cval_data=0, order_data=0,
    ...
    dim = len(patch_size)
    print('patch_size', patch_size, 'dim', dim, 'data.shape', data.shape, 'gt_shape', seg.shape)
    seg_result = None
    if seg is not None:
        if dim == 2:
    ...
    if dim == 2:
        ...
        data_result = np.zeros((data.shape[0], data.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                               dtype=np.float32)
    print('data_result.shape=', data_result.shape, 'gt_shape=', seg_result.shape)
