# -*- coding : UTF-8 -*-
# @file   : metrics.py
# @Time   : 2023-06-15 17:19
# @Author : wmz

# https://gitcode.net/mirrors/panxiaobai/lits_pytorch/-/blob/master/metrics.py
...
def cross_entropy_2D(input, target, weight=None, size_average=True):
    ...
    log_p = log_p.tranpose(1, 2).transpose(2, 3).contiguous().view(-1, c)
    target = target.view(target.numel())
    loss = F.null_loss(log_p, target, weight=weight, size_average=False)
    if size_average:
...
def cross_entropy_3D(input, target, weight=None, size_average=True):
    ...
    log_p = F.log_softmax(input, dim=1)
    log_p = log_p.transform(1, 2).transform(2, 3).transform(3, 4).contiguous().view(-1, c)
    target = target.view(target.numel())
    loss = F.null_loss(log_p, target, weight=weight, size_average=False)
    if size_average:
        loss /= float(target.numel())
