...
from augmentations.spatial_transformations import augment_spatial, augment_spatial_2, \
    augment_channel_translation, translation_alone_RAS_LPI, augment_rotation_alone_axis, \
...
class RotateAxisTransform(AbstractTransform):
    def __init__(self, x_angle_range=(0, 1), y_angle_range=(0, 1), z_angle_range=(0, 1), axes=(0, 1, 2), data_key="data", label_key="gt", p_per_sample=0.5):
        """
        :param axes: around which axes will the rotation take place? two axes are chosen randomly from axes.
                        here, dimensions of input data is [B,C,X,Y,Z], but axes=(0,1,2) means axes=(x,y,z)
                        for be easy to understand and to use, we just ignore Batch_size and Channel. So please
                        note it when use this function..... [Add by Ding]..
        :param data_key:
        :param label_key:
        :param p_per_sample:
        """
        ...
        self.along_x_axis = (0 in self.axes)
        self.along_y_axis = (1 in self.axes)
        self.along_z_axis = (2 in self.axes)

    def __call__(self, **data_dict):
        data = data_dict.get(self.data_key)
        heatmap = data_dict.get(self.label_key)
        a_x = a_y = a_z = 0
        ...
        if self.along_x_axis:
            x_angle = np.random.randint(self.x_angle_range[0], self.x_angle_range[1])
            a_x = x_angle / 180*np.pi
        if self.along_y_axis:
            y_angle = np.random.randint(self.y_angle_range[0], self.y_angle_range[1])
            a_y = y_angle / 180*np.pi
        if self.along_z_axis:
            z_angle = np.random.randint(self.z_angle_range[0], self.z_angle_range[1])
            a_z = z_angle / 180*np.pi

        for b in range(data.shape[0]):
            for c in range(data.shape[1]):
                data[b, c] = augment_rotation_alone_axis(data[b, c], a_z, a_y, a_x, mode='constant',
                                                         cval=data[b , c].min())
        if heatmap is not None:
            for b in range(heatmap.shape[0]):
                for c in range(heatmap.shape[1]):
                    heatmap[b, c] = augment_rotation_alone_axis(heatmap[b, c], a_z, a_y, a_x, mode='constant', cval=0)

        data_dict[self.data_key] = data
        if heatmap is not None:
            data_dict[self.label_key] = heatmap
        return data_dict


class RotateAxisTransformExtent(AbstractTransform):
    def __init__(self, x_angle_range=(0, 1), y_angle_range=(0, 1), z_angle_range=(0, 1), axes=(0, 1, 2), data_key="data", label_key="gt", p_per_sample=0.5):
        """
        :param axes: around which axes will the rotation take place? two axes are chosen randomly from axes.
                        here, dimensions of input data is [B,C,X,Y,Z], but axes=(0,1,2) means axes=(x,y,z)
                        for be easy to understand and to use, we just ignore Batch_size and Channel. So please
                        note it when use this function..... [ acquire extent data]..
        :param data_key:
        :param label_key:
        :param p_per_sample:
        """
...
## Added by  Ding
class SampleTranslation(AbstractTransform):
    """
    Args: [B,C,X,Y,Z]

    """
    def __init__(self, data_key="data", label_key="gt", direction_and_step_dict=None):
        self.data_key = data_key
        self.label_key = label_key
        self.direction_and_step_dict = direction_and_step_dict
        self.directions = list(direction_and_step_dict.keys())
    ...
    def __call__(self, **data_dict):
        ...
        trans_step = np.random.randint(self.direction_and_step_dict[direction])
        print("direction = ", direction, "steps = ", trans_step)
        ret_val = translation_alone_RAS_LPI(data[0], direction, trans_step)
        data_dict[self.data_key][0] = ret_val
        print('data.shape =  ', ret_val.shape)
        if heatmap is not None:
            assert heatmap.shape[0] == 1
            ret_val = translation_alone_RAS_LPI(heatmap[0], direction, trans_step)
            data_dict[self.label_key][0] = ret_val
            print('heatmap.shape =  ', ret_val.shape)
        return data_dict
